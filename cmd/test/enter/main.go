...
func main() {
	...
	fmt.Println("starting builtin auth server")
	builtinAuthServer := frame_auth.AuthServer{}
	authServer, err := builtinAuthServer.Start(ctx)
	if err != nil {
		log.Fatal(err)
	}
	defer builtinAuthServer.Stop()
	roomID := strings.TrimSpace(envOr("TAN_LOBBY_ROOM_ID", "370173"))
	fmt.Println("connecting tan lobby room:", roomID)

	tf := frame.TaskFrameConfig{Embedded: true}.New(nil)
	if err := tf.Connect(ctx, define.ConnectConfig{
		AuthServer: authServer,
		AuthToken:  cookie,
		ServerCode: "TanLobby:" + roomID,
	}); err != nil {
		log.Fatal(err)
	}
